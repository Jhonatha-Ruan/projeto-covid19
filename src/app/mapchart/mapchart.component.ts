...
export class MapchartComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  lineBorderColor = '#080808a8';
  lineStrongerBorderColor = '#080808a8';
  ...
  ngOnInit() {
    ...
    Promise.all(dataPromises).then(values => {
      ...
      // console.log( self.data['2022-03-27']['estados']['MT'].total_death);
  ...
  getPlasmaList = cant => {
    ...
    return ['#ffffd9', '#edf8b1', '#F0E68C',  '#ffae43', '#FFA500',
       '#FF8C00', '#ff6e0b', '#FF4500', '#8B0000'];
  };
  ...
  loadWidgetCountry = (byTrend = false, byDeaths = false, byDensity = false, byNewCases = false) => {
    ...
    self.tipCountry.attr('class', 'd3-tip')
        ...
        .html(function(d) {
          ...
          return (
            '<div style="opacity:0.8;background-color:#0e0e0ee8;padding:7px;color:white">' +
    ...
    for (const item in self.rankingStates) {
      ...
      statesRankingElmnt
        ...
        .html(
          '<td class=" gt-ranking-number"  style="padding-left: 11px; text-align: right; color: #c90000; font-weight: 650;">' +
  ...
  loadWidgetState = (stateParam, byTrend = false, byDeaths = false, byDensity = false, byNewCases = false) => {
    ...
    for (const item in self.rankingCounties) {
      countiesRankingElmnt
        .append('tr')
        .html(
            '<td class=" gt-ranking-number"  style="padding-left: 6px; text-align: right; color: #c90000; font-weight: 650;">' +
